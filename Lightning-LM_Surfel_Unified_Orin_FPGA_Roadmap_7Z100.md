...
## Stage 66 Implementation: EKF Update Stability Gate and Timing Closure

Stage66 Windows-side support was implemented on 2026-07-18.

Stage66A adds repeated EKF update golden support on Orin:

- `run_mapping_ekf_update_xdma_golden` now accepts `--repeat N`.
- Each iteration rewrites EKF input/output buffers, dispatches `KERNEL_SEL=5`,
  checks `STATUS/ERROR`, verifies `RUN_COUNT`, compares `dx/state/cov`, and
  optionally writes `ekf_update_xdma_output_iter_XX.json`.
- New markers:
  - `MAPPING_EKF_UPDATE_REPEAT_ITER_PASS i/N`
  - `MAPPING_EKF_UPDATE_REPEAT_PASS repeat=N`

Stage66A Orin command:
...
- 50/50 iterations must output `MAPPING_EKF_UPDATE_XDMA_NUMERIC_PASS`.
- Every iteration must output `MAPPING_EKF_UPDATE_REPEAT_ITER_PASS i/50`.
- Final marker must be `MAPPING_EKF_UPDATE_REPEAT_PASS repeat=50`.
- `RUN_COUNT` must increase every iteration.
- Tolerances remain `dx/state abs <= 1e-9`, `cov abs <= 1e-8`.
- No new XDMA config BAR failure, `CmpltTO`, AER fatal, offline, or frozen.

Stage66B Windows timing closure attempt:
...
Stage66B Windows result on 2026-07-18:
...
Vivado HLS C Synthesis: PASS
Vivado HLS IP export: PASS
EKF HLS target clock: 8.00 ns
EKF HLS estimated clock: 7.673 ns
EKF HLS resources: BRAM_18K=94, DSP48E=407, FF=75279, LUT=79138
BD validate: PASS
Project synthesis: PASS
Implementation / bitstream: PASS
Bitstream: fpga/vivado/.build/azmig_impl/azmig.runs/impl_1/azmig_wrapper.bit
Post-route WNS=-0.575 ns
Post-route TNS=-32.114 ns
Post-route WHS=0.021 ns
Post-route THS=0.000 ns
Timing constraints are not met.
Utilization: LUT=146962 (52.98%), FF=164410 (29.63%), BRAM tile=127.5 (16.89%), DSP=792 (39.21%)
DRC: 0 errors, Vivado implementation log reports 1 critical warning.
```

The Stage66B bitstream is better than the Stage65B timing-risk image
(`TNS=-2451.445 ns` previously), but it is still not timing-closed. The worst
setup path is currently in `unified_obs_0`, not `slam_ekf_update_core`:

```text
Path group: userclk2
Worst path: unified_obs_0 double multiply DSP chain
Data path delay: 8.432 ns against 8.000 ns
```

Conclusion:

- Stage66A repeated EKF stability can now be run on Orin with the generated
  Stage66B bitstream as a short functional smoke.
- Stage66B does not satisfy the reliable engineering bitstream gate because
  post-route setup timing is still negative.
- Online `FPGA_FULL` remains disabled.
- Next timing closure should prioritize the observation HLS double multiply
  path or clock-domain/frequency partitioning, because the EKF HLS core itself
  meets its 8ns HLS estimate.

Stage66 reports:

- `reports/fpga/runtime/stage66_ekf_update_stability/`
- `reports/fpga/hls/ekf_update_core/stage66_timing_closure/`
- `reports/fpga/vivado/stage66_timing_closure/`

## Stage 67 Implementation: Full Design Timing Closure

Stage67 Windows-side timing closure was completed on 2026-07-18.

Stage67 does not add new runtime features. It fixes the timing risk left by
Stage66B before continuing toward online `FPGA_FULL`.

Root cause:

- Stage66B had already retimed `slam_ekf_update_core` for 8 ns, but the worst
  post-route path was not inside EKF.
- The failing path was inside `unified_obs_0`, specifically a double multiply
  DSP chain generated by `unified_surfel_observation_core`.
- The observation HLS flow was still creating the clock with a fixed 10 ns
  target while the board-level XDMA user clock is 8 ns / 125 MHz.

Stage67 change:

- `fpga/hls/unified_surfel_observation_core/create_vivado_hls_project.tcl`
  now accepts a fifth `target_clock_ns` argument.
- `run_vivado_hls_csim.ps1`, `run_vivado_hls_csynth.ps1`, and
  `run_vivado_hls_export_ip.ps1` now default to `-ClockNs 8`.
- Mathematical behavior, ABI, BAR shim, XDMA/MIG layout, controller register
  map, `KERNEL_SEL=4/5`, and Orin runtime contracts are unchanged.

Stage67 Windows commands:
...
Stage67 Windows result:

```text
Observation g++ CSim: PASS
Observation Vivado HLS CSim: PASS
Observation Vivado HLS C Synthesis: PASS
Observation Vivado HLS IP export: PASS
Observation HLS target clock: 8.00 ns
Observation HLS estimated clock: 7.436 ns
Observation HLS resources: BRAM_18K=194, DSP48E=365, FF=85420, LUT=118553
BD validate: PASS
Project synthesis: PASS
Implementation / bitstream: PASS
Bitstream: fpga/vivado/.build/azmig_impl/azmig.runs/impl_1/azmig_wrapper.bit
...
Post-route setup failing endpoints=0
Post-route WHS=0.016 ns
Post-route THS=0.000 ns
Post-route hold failing endpoints=0
Timing: all user specified timing constraints are met
DRC: 0 errors, 0 critical warnings
Utilization: LUT=147113 (53.03%), FF=171772 (30.96%), BRAM tile=127.5 (16.89%), DSP=792 (39.21%)
```

Conclusion:

- Stage67 turns the Stage66B timing-risk bitstream into a timing-clean
  engineering bitstream.
- The previous `unified_obs_0` double multiply timing violation is resolved by
  regenerating the observation HLS IP against the real 8 ns board clock.
- Online `FPGA_FULL` is still not enabled immediately. The next gate is Orin
  regression on the timing-clean bitstream.

Stage67 JTAG command:

```powershell
powershell -ExecutionPolicy Bypass -File .\fpga\vivado\slam_accel_ax7z100_pcie_mig\program_bitstream_jtag.ps1 -Bitstream .\fpga\vivado\.build\azmig_impl\azmig.runs\impl_1\azmig_wrapper.bit
```

Stage67 Orin regression commands:
...
Stage67 Orin acceptance:

- XDMA shim/reg/DDR smoke must still PASS.
- Localization observation V2 must still PASS.
- Mapping observation V2 must still PASS.
- EKF update repeated stability must output
