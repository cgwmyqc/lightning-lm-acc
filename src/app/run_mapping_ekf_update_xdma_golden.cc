...
DEFINE_string(output_dir, "", "Directory for Stage65C JSON output");
...
bool WriteResultJson(const std::string& output_dir, const std::string& golden_dir, uint32_t ctrl_base,
    ...
    const auto path = std::filesystem::path(output_dir) / "ekf_update_xdma_output.json";
...
int main(int argc, char** argv) {
    ...
    LOG(INFO) << "[mapping_ekf_update_xdma] STATUS=0x" << std::hex << result.status
              << " ERROR=0x" << result.error << std::dec
              << " RUN_COUNT=" << result.run_count_before << "->" << result.run_count_after
              << " hls_wait_sec=" << result.timing.hls_wait_sec << " " << report;

    if (!WriteResultJson(FLAGS_output_dir, FLAGS_golden_dir, options.ctrl_base, result, frame.expected, report,
                         &error)) {
        LOG(ERROR) << error;
        return 3;
    }
