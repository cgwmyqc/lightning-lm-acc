...
if ([string]::IsNullOrWhiteSpace($ProjectDir)) {
    $ProjectDir = Join-Path $DefaultBuildRoot "hls_slam_loc_iterative_csim"
}
...
try {
    ...
    & $VivadoHls -f $Tcl -tclargs $GoldenDir $VivadoProjectDir $Part "csim" $ClockNs
    if ($LASTEXITCODE -eq 0) {
        Write-Host "LOC_ITER_HLS_CSIM_PASS"
    }
    exit $LASTEXITCODE
} finally {
