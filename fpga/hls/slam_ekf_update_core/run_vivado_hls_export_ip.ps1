...
if ([string]::IsNullOrWhiteSpace($ProjectDir)) {
    $ProjectDir = Join-Path $DefaultBuildRoot "hls_slam_ekf_update_export"
}
...
try {
    ...
    if ((Test-Path $ComponentXml) -or ((Get-ChildItem $ZipPattern -ErrorAction SilentlyContinue | Select-Object -First 1) -ne $null)) {
        Write-Host "HLS_EKF_UPDATE_EXPORT_IP_PASS"
        Write-Host "EKF_HLS_IP_DIR=$IpDir"
        exit 0
    }
...
$ComponentXml = Join-Path $ProjectDir "solution1\impl\ip\component.xml"
if (!(Test-Path $ComponentXml)) {
    Write-Error "EKF HLS IP export did not produce component.xml: $ComponentXml"
    exit 1
}

Write-Host "HLS_EKF_UPDATE_EXPORT_IP_PASS"
Write-Host "EKF_HLS_IP_DIR=$(Split-Path -Parent $ComponentXml)"
