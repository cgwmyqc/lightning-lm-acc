...
set project_dir [file normalize [file join $script_dir ".." ".build" "xdma_restore_stage_a2_impl"]]
...
if {![catch {open_run impl_1} open_err]} {
    write_report_if_possible "post-implementation utilization" \
        [list report_utilization -file [file join $project_dir "xdma_restore_stage_${stage_lc}_impl_util.rpt"]]
    write_report_if_possible "post-implementation timing" \
        [list report_timing_summary -file [file join $project_dir "xdma_restore_stage_${stage_lc}_impl_timing.rpt"]]
    write_report_if_possible "post-implementation DRC" \
        [list report_drc -file [file join $project_dir "xdma_restore_stage_${stage_lc}_impl_drc.rpt"]]
    write_report_if_possible "post-implementation route status" \
        [list report_route_status -file [file join $project_dir "xdma_restore_stage_${stage_lc}_impl_route_status.rpt"]]
} else {
    puts "WARN: failed to open impl_1 for reports: $open_err"
}

set bit_files [glob -nocomplain -directory [file join $project_dir "xdma_restore_stage_${stage_lc}.runs" "impl_1"] "*.bit"]
set bit_path ""
if {[llength $bit_files] > 0} {
    set bit_path [file normalize [lindex $bit_files 0]]
}

set bit_info [open [file join $project_dir "xdma_restore_stage_${stage_lc}_bitstream_path.txt"] w]
...
puts "XDMA_RESTORE_STAGE_${stage}_IMPLEMENTATION_BITSTREAM_PASS"
exit
