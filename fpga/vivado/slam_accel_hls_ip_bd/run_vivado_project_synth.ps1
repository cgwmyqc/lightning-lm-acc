...
if ([string]::IsNullOrWhiteSpace($ProjectDir)) {
    $ProjectDir = Join-Path $env:TEMP "lightning_slam_accel_hls_ip_bd_project_synth"
}
...
foreach ($Item in @(
    ...
    @{ Source = (Join-Path $ProjectDir "slam_accel_hls_ip_bd_project_synth_utilization.rpt"); Target = "slam_accel_hls_ip_bd_project_synth_utilization.txt" },
    @{ Source = (Join-Path $ProjectDir "slam_accel_hls_ip_bd_project_synth_timing_summary.rpt"); Target = "slam_accel_hls_ip_bd_project_synth_timing_summary.txt" },
    @{ Source = (Join-Path $ProjectDir "slam_accel_hls_ip_bd_ip_status.rpt"); Target = "slam_accel_hls_ip_bd_ip_status_project_synth.txt" },
    @{ Source = (Join-Path $ProjectDir "slam_accel_hls_ip_bd.runs\synth_1\runme.log"); Target = "vivado_project_synth_runme_log.txt" }
)) {
