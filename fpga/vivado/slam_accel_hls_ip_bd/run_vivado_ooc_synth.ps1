...
if ([string]::IsNullOrWhiteSpace($ProjectDir)) {
    $ProjectDir = Join-Path $env:TEMP "lightning_slam_accel_hls_ip_bd_ooc"
}
...
$Log = Join-Path $ProjectDir "vivado_ooc_synth.log"
$Journal = Join-Path $ProjectDir "vivado_ooc_synth.jou"
...
foreach ($Item in @(
    @{ Source = $Log; Target = "vivado_ooc_synth_log.txt" },
    @{ Source = $Journal; Target = "vivado_ooc_synth_jou.txt" },
    @{ Source = (Join-Path $ProjectDir "slam_accel_hls_ip_bd_utilization.rpt"); Target = "slam_accel_hls_ip_bd_utilization.txt" },
    @{ Source = (Join-Path $ProjectDir "slam_accel_hls_ip_bd_timing_summary.rpt"); Target = "slam_accel_hls_ip_bd_timing_summary.txt" },
    @{ Source = (Join-Path $ProjectDir "slam_accel_hls_ip_bd_ip_status.rpt"); Target = "slam_accel_hls_ip_bd_ip_status_ooc.txt" }
)) {
